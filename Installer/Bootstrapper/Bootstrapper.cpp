...
void Bootstrapper::RegisterUninstall(const TCHAR *productName)
{
	...
	throwHRESULT (keyProductCode.SetStringValue(_T("URLInfoAbout"), _T("http://www.watrbx.wtf")), "Failed to set URLInfoAbout key");
...
void Bootstrapper::setLatestProcess()
{
	...
	const std::string latestProcessName = format_string("www.watrbx.wtf/%s/%s/latestProcess", installHost.c_str(), Type().c_str());
