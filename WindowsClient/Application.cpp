...
Application::RequestPlaceInfoResult Application::requestPlaceInfo(const std::string url, std::string& authenticationUrl, std::string& ticket, std::string& scriptUrl) const
{
	try
	{
		...
		if (parseResult)
		{
			...
			if (status == 2)
			{
				currentDocument->SetUiMessage("Place Loaded! Joining...");
			...
			else
			{
				...
				if (status == 1) {
					currentDocument->SetUiMessage("A server is loading the game...");
					return RETRY;
				}
			}
		}
...
{
	RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "Important diddy detected.");
}
